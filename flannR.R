...
if( "testShiftNN" %in% ls() ) {
  if( testShiftNN ) {
    ...
    index <- sample( 1:nrow(x), 10 ) 
    train <- x[index,]
   
    nn <- getNN(train, train, 5)
    print( nn$query ) 

    for( i in 1:10 ) {
      nn <- getNN(nn$query, train, 5)
      print( nn$query )
    }
...
#    r.new.values[r.values.finite] <- nn$query[,3]
#    values(r.new) <- r.new.values
#    
#    r.new.var <-r$var
#    r.new.values <- values(r.new) 
#    r.new.values[r.values.finite] <- nn$query[,4]
